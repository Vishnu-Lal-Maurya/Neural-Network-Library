...
namespace NN{
    ...
    void assertLength(const row& v1, const row& v2){
        std::size_t length1{ v1.size() };
        std::size_t length2{ v2.size() };
        assert(length1==length2 && "Lenghts Don't match");
    }

    void assertLength(const matrix& v1, const matrix& v2){
        std::size_t rows1{ v1.size() };
        std::size_t rows2{ v2.size() };
        assert(rows1==rows2 && "Dimensions must match");
        assert(rows1!=0 && "Dimension should be non-zero");
    ...
    matrix elementWiseMatrixOperations(const matrix& v1, const matrix& v2, char op){
        ...
        for(int i=0; i<numRows; ++i){
            for(int j=0; j<numCols; ++j){
                ...
                {
                case '+':
                    val = v1[toUZ(i)][toUZ(j)] + v1[toUZ(i)][toUZ(j)];
                    break;
                case '-':
                    val = v1[toUZ(i)][toUZ(j)] - v1[toUZ(i)][toUZ(j)];
                    break;
